...
## 07-22-2026 - Phase 31: Hosted Demo Replay Fix

### What Changed

- Renamed the project-facing title to `Open-Source Tax Tooling - AI Engineering Research`.
- Added a committed verified demo-run payload generated by `python run_day20_demo.py --json`.
- Updated the execution lab API so Vercel returns the verified runner payload instead of a stub response.
- Updated the UI so hosted synthetic runs show passed lifecycle phases, command rows, command output, summary totals, warnings, and failure-matrix results.
- Clarified the README and prototype README hosted-demo behavior.

### Why

The Vercel deployment needs to be an engaging live review surface even when it cannot execute a local hledger binary. The hosted synthetic run now shows the same command/output contract as a local run while preserving the documented boundary that true hledger execution remains local.
...
- `prototype/execution_lab/data/verified-demo-run.json`
...
- `npm run build`
- Vercel-mode runtime smoke test: `POST /api/run` returned HTTP 200 with `status: passed`, two command records, 11 lifecycle steps, 15 safety cases, and 19 transactions.
- Vercel-mode build simulation with `VERCEL=1 npm run build`

### Next

Commit and push the Vercel demo fix so the production deployment can pick it up from Git.
